import {expect} from './chai-setup';
import {ethers, deployments, getUnnamedAccounts, ethereum} from '@nomiclabs/buidler';
import { utils, Wallet } from 'ethers';
const mockCreator = ['https://utulsa.edu/wp-content/uploads/2018/08/generic-avatar.jpg', 'ETHGlobal', 5];

const Transaction = require("ethereumjs-tx").Transaction;
const ethUtils = require("ethereumjs-util");

const assert = require("assert").strict;

describe('CreatonFactory', function () {
  it('deploys successfully', async function () {
    await deployments.fixture();
    const creatonFactory = await ethers.getContract('CreatonFactory');
    expect(creatonFactory.address).to.be.a('string');
  });

  it('deploys Creator contract', async function () {
    await deployments.fixture();
    const creatonFactory = await ethers.getContract('CreatonFactory');
    const creator = await creatonFactory.deployCreator(...mockCreator);
    expect(creator.hash).to.be.a('string');
  });
});

describe('Creator', function () {
  let creator: any;
  async function setup() {
    await deployments.fixture();
    creator = await ethers.getContract('Creator');
    creator.init(...mockCreator);
  }

  beforeEach(setup);

  // it('deploys with all param data set', async function () {
  //   expect(await creator.creatorTitle()).to.equal('ETHGlobal');
  //   expect(await creator.avatarURL()).to.equal('https://utulsa.edu/wp-content/uploads/2018/08/generic-avatar.jpg');
  //   expect(await creator.subscriptionPrice()).to.equal(5);
  // });
  /*
  it('sets avatar url', async function () {
    creator.setAvatarURL(
      'https://image.shutterstock.com/image-vector/hand-drawn-modern-man-avatar-260nw-1373616869.jpg'
    );
    expect(await creator.avatarURL()).to.equal(
      'https://image.shutterstock.com/image-vector/hand-drawn-modern-man-avatar-260nw-1373616869.jpg'
    );
  });*/
});

describe('ERC1820', function () {
  it('deploys successfully', async function ( web3 ) {

    //gasless chain is fine simple
    //for gas chain we need to send eth first

    // Check if erc1820 is allready deployed in this network.
    //ERC820_ADDRESS = "0x1820a4B7618BdE71Dce8cdc73aAB6C95905faD24";
    //const erc1820ContractNounce = await getTransactionCount(ERC820_ADDRESS);
    //if (erc1820ContractNounce === 0) {

    //EASY DEPLOY, MIGHT NOT WORK AS IT NEEDS data: "0x" + require("../src/superfluid/introspection/ERC1820Registry.json").bin, IN TRANSACTION
    /*

    await deployments.fixture();
    const creatonSuperApp = await ethers.getContract('0x1820a4B7618BdE71Dce8cdc73aAB6C95905faD24');
    //const creator = await creatonFactory.deployCreator(...mockCreator);
    
    */

    //MANUAL DEPLOY, NEEDS GAS

      console.log("Static erc1820 deployment initiated");
      const targetAddress = "0xa990077c3205cbDf861e17Fa532eeB069cE9fF96";
      const rawTx = {
        nonce: 0,
        gasPrice: 100000000000,
        value: 0,
        data: "0x" + require("../src/superfluid/introspection/ERC1820Registry.json").bin,
        gasLimit: 800000,
        v: 27,
        r: "0x1820182018201820182018201820182018201820182018201820182018201820",
        s: "0x1820182018201820182018201820182018201820182018201820182018201820"
        
    };

    const tx = new Transaction(rawTx);

    const res = {
      sender: ethUtils.toChecksumAddress(
          "0x" + tx.getSenderAddress().toString("hex")
      ),
      rawTx: "0x" + tx.serialize().toString("hex"),
      contractAddr: ethUtils.toChecksumAddress(
          "0x" + ethUtils.generateAddress(tx.getSenderAddress(), ethUtils.toBuffer(0)).toString("hex")
      ),
  };

  assert.equal("0xa990077c3205cbDf861e17Fa532eeB069cE9fF96", res.sender);
  assert.equal("0x1820a4B7618BdE71Dce8cdc73aAB6C95905faD24", res.contractAddr);

  //expect(await res.sender.to.equal(
  //  '0xa990077c3205cbDf861e17Fa532eeB069cE9fF96'
  //));

  //expect(await res.contractAddr.to.equal(
  //  '0x1820a4B7618BdE71Dce8cdc73aAB6C95905faD24'
  //));

      //sendeth
      const signer = await ethers.getSigners();
      const tx1 = await signer[0].sendTransaction({
        to: res.sender,
        value: utils.parseEther("0.08") //ethers.utils.parseEther("0.08"), 
      });
      await tx1.wait();
      console.log("erc1820 target address funded");
      const tx2 = await ethers.provider.sendTransaction(res.rawTx);
      await tx2.wait();
      //console.log("REACT_APP_ERC1820='" + ERC820_ADDRESS + "'");
      //await saveContractAddress("erc1820", ERC820_ADDRESS);
      console.log("successful erc1820 deploy!")
      

    /*

    console.log("Dynamic erc1820 deployment initiated");
    // We are probably on gasless chain
    // We deploy it manually
    const factory = await ethers.getContract("ERC1820RegistryDeployable");
    let contract = await factory.deploy();
    await contract.deployed();

    /*
    ERC1820_ADDRESS = contract.address;
    console.log("REACT_APP_ERC1820='" + ERC1820_ADDRESS + "'");
    await saveContractAddress("erc1820", ERC1820_ADDRESS);

    expect(await creator.avatarURL()).to.equal(
      'https://image.shutterstock.com/image-vector/hand-drawn-modern-man-avatar-260nw-1373616869.jpg'
    );
    */
    /*
      // Check if gasless chain
      const address = await getAddress();
      const balance = await getBalance(address);
      let ERC820_ADDRESS = "";
      if (balance.isZero()) {
        console.log("Dynamic erc1820 deployment initiated");
        // We are probably on gasless chain
        // We deploy it manually
        const factory = await ethers.getContract("ERC1820RegistryDeployable");
        let contract = await factory.deploy();
        await contract.deployed();
        ERC820_ADDRESS = contract.address;
        console.log("REACT_APP_ERC1820='" + ERC820_ADDRESS + "'");
        await saveContractAddress("erc1820", ERC820_ADDRESS);
      } else {
        // Check if erc1820 is allready deployed in this network.
        ERC820_ADDRESS = "0x1820a4B7618BdE71Dce8cdc73aAB6C95905faD24";
        const erc1820ContractNounce = await getTransactionCount(ERC820_ADDRESS);
        if (erc1820ContractNounce === 0) {
          console.log("Static erc1820 deployment initiated");
          const targetAddress = "0xa990077c3205cbDf861e17Fa532eeB069cE9fF96";
          const rawTx =
            "";
          const signer = await getSigner();
          const tx1 = await signer.sendTransaction({
            to: targetAddress,
            value: ethers.utils.parseEther("0.08"),
          });
          await tx1.wait();
          console.log("erc1820 target address funded");
          const tx2 = await ethers.provider.sendTransaction(rawTx);
          await tx2.wait();
          console.log("REACT_APP_ERC1820='" + ERC820_ADDRESS + "'");
          await saveContractAddress("erc1820", ERC820_ADDRESS);
        } else {
          console.log("erc1820 allready deployed on network");
        }
      }*/
      //return ERC820_ADDRESS;

    //await deployments.fixture();
    //const creatonFactory = await ethers.getContract('CreatonFactory');
    //expect(creatonFactory.address).to.be.a('string');
  });

});
